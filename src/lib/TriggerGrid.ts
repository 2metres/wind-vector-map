...
export class TriggerGrid {
  ...
  private armDisc(cx: number, cy: number, radius: number, dirX: number, dirY: number) {
  ...
  armLine(fromCol: number, fromRow: number, toCol: number, toRow: number, dirX: number, dirY: number, brushRadius = 3) {
